    ...
    def next(self):
        ...
        if self.position:
            if self.entry_bar is not None and (len(self) - self.entry_bar) >= self.params.hold_days:
                ...
                print(f'{self.data.datetime.date(0)}, 平仓:  当前价格: {self.data.close[0]}, 利润: {profit:.2f}')
                self.close()
                self.entry_bar = None
            return  # 平仓后本bar不再开新仓
        ...
        if self.breakout_bar is not None:
            ...
            if self.data.close[0] < breakout_open:
                print(f'{self.data.datetime.date(0)}, 回调失败, 取消等待')
                self.breakout_bar = None
            # 回调周期结束且价格仍在阻力位上方，买入
            elif bars_since_breakout >= self.params.pullback_period:
                self.order = self.buy()
                self.entry_bar = len(self)
                print(f'{self.data.datetime.date(1)}, 开多单: 当前价格: {self.data.open[1]}, 前日收盘价: {self.data.close[0]}, 阻力位: {self.resistance[0]}')
                self.breakout_bar = None
            return
        ...
        if self.breakdown_bar is not None:
            ...
            # 回调周期结束且价格仍在支撑位下方，卖出开空
                ...
                self.entry_bar = len(self)
                print(f'{self.data.datetime.date(1)}, 开空单: 当前价格: {self.data.open[1]}, 前日收盘价: {self.data.close[0]}, 支撑位: {self.support[0]}')
                self.breakdown_bar = None
            return
        ...
        if self.data.close[0] > self.resistance[0]:
            ...
            print(f'{self.data.datetime.date(0)}, 突破阻力位，等待回调确认: 当前价格: {self.data.close[0]}, 阻力位: {self.resistance[0]}')
            return

        # 跌破支撑位逻辑
        if self.data.close[0] < self.support[0]:
            self.breakdown_bar = len(self)
            print(f'{self.data.datetime.date(0)}, 跌破支撑位，等待回调确认: 当前价格: {self.data.close[0]}, 支撑位: {self.support[0]}')
            return
